...
export default function EditType({ type }: Props) {
  ...
  const submit: FormEventHandler = (e) => {
    e.preventDefault();
    put(route('admin.event.update', type.id), {
      onSuccess: () => toast.success('Event Type updated successfully!'),
  ...
  return (
    <AppLayout>
      <Head title={`Edit Event Type: ${type.name}`} />
      ...
      <div className="mx-5 p-6 shadow rounded">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <Button type="submit" disabled={processing}>
            {processing ? 'Updating...' : 'Update Event Type'}
          </Button>
        </form>
      </div>
    </AppLayout>
